...
const isMockMode = (process.env.RAZORPAY_KEY_SECRET === 'test_mode' || !process.env.RAZORPAY_KEY_SECRET || process.env.RAZORPAY_KEY_SECRET.includes('your_actual'));
if (isMockMode) {
  console.log('⚠️  MOCK MODE: Using test orders (Razorpay credentials not configured)');
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const user = await db.get('SELECT id, name, email, password_hash, role FROM Users WHERE email = ?', [email]);
...
// Add family member
app.post('/api/family-members', verifyToken, upload.single('photo'), async (req, res) => {
  try {
    ...
    const member = await db.get('SELECT id, name, date_of_birth, photo_path FROM FamilyMembers WHERE id = ?', [result.lastID]);
...
app.get('/api/plans', async (req, res) => {
  try {
    const plans = await db.all('SELECT id, name, child_price, adult_price, senior_price, duration_days FROM Plans');
    res.json(plans);
  } catch (err) {
    console.error(err);
...
app.post('/api/create-order', verifyToken, async (req, res) => {
  try {
    ...
    if (!family_member_id || !plan_id) {
      return res.status(400).json({ error: 'Family member and plan are required' });
    }
    ...
    // Get plan
    const plan = await db.get('SELECT id, name, child_price, adult_price, senior_price, duration_days FROM Plans WHERE id = ?', [plan_id]);
    ...
    const hasBirthday = (today.getMonth() > dob.getMonth()) || (today.getMonth() === dob.getMonth() && today.getDate() >= dob.getDate());
    if (!hasBirthday) age--;
    ...
    try {
      ...
      return res.status(500).json({ error: `Payment gateway error: ${paymentErr.message}. Make sure RAZORPAY_KEY_SECRET is set in .env` });
    }
...
app.post('/api/verify-payment', verifyToken, async (req, res) => {
  try {
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, family_member_id, plan_id } = req.body;
    ...
    const member = await db.get('SELECT user_id FROM FamilyMembers WHERE id = ?', [family_member_id]);
    const plan = await db.get('SELECT duration_days FROM Plans WHERE id = ?', [plan_id]);
    ...
    const subscription = await db.run(
      ...
      [family_member_id, plan_id, startDate, endDate, razorpay_payment_id || `mock_${Date.now()}`, 'active']
    );
    ...
    const qrDataUrl = await QRCode.toDataURL(qrToken, { errorCorrectionLevel: 'L', type: 'image/png', width: 300 });
...
app.get('/api/my-qrcodes', verifyToken, async (req, res) => {
  try {
    ...
    // Generate QR images
    const qrCodesWithImages = await Promise.all(qrcodes.map((qr) => {
      return QRCode.toDataURL(qr.token, { errorCorrectionLevel: 'L', type: 'image/png', width: 200 }).then((qrDataUrl) => ({ ...qr, qrImage: qrDataUrl }));
    }));
...
app.get('/api/admin/members-subscriptions', verifyToken, verifyAdmin, async (req, res) => {
  try {
...
app.post('/api/qr/validate', verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const hasBirthday = (todayDate.getMonth() > dob.getMonth()) || (todayDate.getMonth() === dob.getMonth() && todayDate.getDate() >= dob.getDate());
    if (!hasBirthday) age--;
